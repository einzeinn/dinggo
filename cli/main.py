...
def _handle_subcommands(working_dir: str) -> bool:
    """Handles subcommands if provided in sys.argv. Returns True if a subcommand was executed."""
    if len(sys.argv) <= 1:
        return False
    ...
    if cmd in ("interface", "tui", "menu", "factory"):
        ...
        from core.planner import Planner as PlanEngine
        from rich.console import Console
        console = Console()
        spec = SpecParser(target_dir).parse()
        planner_inst = PlanEngine()
        res = planner_inst.create_product_task_graph(spec)
